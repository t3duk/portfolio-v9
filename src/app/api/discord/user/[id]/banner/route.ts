...
export const GET = async (
  ...
  const size = request.nextUrl.searchParams.get("size") || "1024";
  ...
  const returnUrl = `https://cdn.discordapp.com/banners/${discordId}/${result.data.banner}?size=${size}`;
  const image = await fetch(returnUrl, {
    next: {
      revalidate: 60 * 60 * 24 * 7,
      tags: ["all", "discord", "discord:banner", `discord:banner:${discordId}`],
    },
  });
